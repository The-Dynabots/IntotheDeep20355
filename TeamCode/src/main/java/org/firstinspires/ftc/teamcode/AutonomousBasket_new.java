...
@Autonomous(name="AutonomousBasket_new", group="Robot")
public class AutonomousBasket_new extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        foldable.setPower(.4);
        waits(1000);
        foldable.setPower(-.06);
        moveForward(30,.5);
        sleep(1000);
        turntime_right(.6,300);
        ...
        while (Math.abs(positionfold) < 1600) {
            positionfold = encoderFold.getCurrentPosition() - initial_positionFold;
            foldable.setPower(.3);
        }
        foldable.setPower(-0.1);
        joint.setPosition(.195);
        claw.setPower(1);
        waits(400);
        claw.setPower(0);
        while(Math.abs(positionfold)<2750) {
            positionfold = encoderFold.getCurrentPosition() - initial_positionFold;
            foldable.setPower(.17);
        ...
        while (Math.abs(positionfold) > 700) {
            positionfold = encoderFold.getCurrentPosition() - initial_positionFold;
            foldable.setPower(-.3);
        }
        foldable.setPower(0);
